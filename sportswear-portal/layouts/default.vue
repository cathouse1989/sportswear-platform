    ...
    <header class="sticky top-0 z-40 bg-white/95 backdrop-blur-lg border-b border-[#EAE5DD]">
      ...
      <transition name="drawer">
        <div v-if="mobileMenuOpen" class="lg:hidden fixed top-0 right-0 bottom-0 z-[60] w-[86vw] max-w-[92vw] sm:w-[360px] sm:max-w-[380px] md:w-[420px] md:max-w-[440px] bg-white shadow-2xl flex flex-col">
          ...
          <div class="px-4 py-6 border-t border-[#EAE5DD]">
            <NuxtLink :to="localePath('/contact')" @click="mobileMenuOpen = false"
              class="block w-full px-4 py-3.5 rounded-xl text-sm font-semibold bg-[#D4A853] text-white text-center min-h-[48px] flex items-center justify-center gap-2">
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" /></svg>
              {{ $t('home.get_quote') }}
            </NuxtLink>
          </div>
