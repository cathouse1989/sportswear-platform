          ...
          <div>
            ...
            <ul class="space-y-3 text-sm text-[#9A8C7A]">
              <li class="flex items-center gap-3">
                ...
                <a :href="contactEmailHref" class="hover:text-white">{{ contactEmail }}</a>
              </li>
              <li class="flex items-center gap-3">
                ...
                <a :href="contactPhoneHref" class="hover:text-white">{{ contactPhone }}</a>
              </li>
              <li class="flex items-center gap-3">
...
// 联系方式（Footer「Contact Info」区块，来自主题配置「联系方式」，回退硬编码默认值）
const contactEmail = computed(() => theme.value.contact_email || 'info@sportswear.com')
const contactPhone = computed(() => theme.value.contact_phone || '+86 123 4567 8900')
const contactAddress = computed(() => theme.value.contact_address || 'Guangzhou, China')
const contactEmailHref = computed(() => 'mailto:' + contactEmail.value)
const contactPhoneHref = computed(() => 'tel:' + contactPhone.value.replace(/[^\d+]/g, ''))
