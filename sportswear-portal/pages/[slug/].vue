  ...
  <div>
    ...
    <section v-if="page?.modules?.length" class="max-w-7xl mx-auto px-4 lg:px-8 py-12 md:py-16 space-y-12">
      <template v-for="m in visibleModules" :key="m.id">
        ...
        <!-- 产品推荐模块 / OEM / ODM / 工厂 / 案例 / 认证 / 博客 -->
        <div v-else-if="['product_recommend', 'oem', 'odm', 'factory', 'production', 'case', 'certification', 'blog'].includes(m.type)"
          class="bg-white rounded-2xl p-8 border border-[#EAE5DD]">
        ...
        <!-- 联系模块 -->
        <div v-else-if="m.type === 'contact'"
          class="bg-gradient-to-r from-[#F5F0E8] to-[#FBF9F6] rounded-2xl p-8 text-center">
    ...
    <!-- Loading -->
    <section v-if="pending" class="max-w-7xl mx-auto px-4 lg:px-8 py-20 text-center text-gray-400">
      加载中...
    </section>
...
const { data: page, pending, error } = await useAsyncData<any>(
