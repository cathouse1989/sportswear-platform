...
// ListProductionProcesses 生产流程列表（后台：全部状态）
func (s *CMSService) ListProductionProcesses() ([]models.ProductionProcess, error) {
	var processes []models.ProductionProcess
	err := s.db.Order("sort_order ASC").Find(&processes).Error
...
func (s *CMSService) CreateProductionProcess(req *ProductionProcessRequest) (*models.ProductionProcess, error) {
	process := models.ProductionProcess{
		Name:        req.Name,
		Status:      req.Status,
		Description: req.Description,
		Image:       req.Image,
...
func (s *CMSService) UpdateProductionProcess(id string, req *ProductionProcessRequest) (*models.ProductionProcess, error) {
	var process models.ProductionProcess
	if err := s.db.First(&process, "id = ?", id).Error; err != nil {
	...
	if err := s.db.Model(&process).Updates(updates).Error; err != nil {
		return nil, err
	}
	return &process, nil
}
